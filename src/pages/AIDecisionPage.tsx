...
const categoryLabels: Record<string, string> = {
  rhythm: '教学节奏调整', task: '任务设计优化',
  evaluation: '评价方式改革', resource: '资源更新建议',
};
...
export default function AIDecisionPage() {
  ...
  const handleDecision = async (suggestionId: string, action: string) => {
    ...
    setSuccessMsg(`AI建议已${actionLabel}，教学干预日志已生成。决策反馈已反哺 AI 训练。`);
    setShowSuccess(true);
  ...
  const confidenceColor = (score: number) => score >= 0.8 ? 'text-emerald-600' : score >= 0.6 ? 'text-orange-500' : 'text-red-500';
  const confidenceBg = (score: number) => score >= 0.8 ? 'bg-emerald-100' : score >= 0.6 ? 'bg-orange-100' : 'bg-red-100';
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-slate-500 mt-1">人机协同驱动的课程优化决策 — AI分析与教师专业经验深度融合</p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        <div className="bg-white rounded-xl border border-slate-200 shadow-sm p-5 flex items-center gap-4">
          <div className="p-3 bg-blue-100 rounded-lg"><Brain size={24} className="text-blue-600" /></div>
          <div><p className="text-2xl font-bold text-slate-800">{suggestions.length}</p><p className="text-sm text-slate-500">AI建议总数</p></div>
        </div>
        <div className="bg-white rounded-xl border border-slate-200 shadow-sm p-5 flex items-center gap-4">
          <div className="p-3 bg-orange-100 rounded-lg"><Clock size={24} className="text-orange-600" /></div>
          <div><p className="text-2xl font-bold text-slate-800">{pendingSuggestions.length}</p><p className="text-sm text-slate-500">待处理</p></div>
        </div>
        <div className="bg-white rounded-xl border border-slate-200 shadow-sm p-5 flex items-center gap-4">
          <div className="p-3 bg-emerald-100 rounded-lg"><Check size={24} className="text-emerald-600" /></div>
          <div><p className="text-2xl font-bold text-slate-800">{decisionLogs.length}</p><p className="text-sm text-slate-500">决策日志</p></div>
        </div>
        <div className="bg-white rounded-xl border border-slate-200 shadow-sm p-5 flex items-center gap-4">
          <div className="p-3 bg-purple-100 rounded-lg"><Sparkles size={24} className="text-purple-600" /></div>
          <div><p className="text-2xl font-bold text-slate-800">{Math.round(suggestions.reduce((s, sug) => s + sug.confidenceScore, 0) / Math.max(suggestions.length, 1) * 100)}%</p><p className="text-sm text-slate-500">AI平均置信度</p></div>
        </div>
      </div>
                ...
                <div className="p-4">
                  <div className="flex items-start justify-between mb-2">
                    <div className="flex items-center gap-2 flex-wrap">
                      <span className={`text-xs px-2 py-0.5 rounded-full font-medium ${priorityConfig[sug.priority].bg}`}>{priorityConfig[sug.priority].label}</span>
                      <span className="text-xs px-2 py-0.5 bg-slate-100 text-slate-600 rounded-full">{categoryLabels[sug.category]}</span>
                      <span className="text-xs text-slate-400">第{sug.week}周</span>
                      <span className={`text-xs px-2 py-0.5 rounded-full font-medium ${confidenceBg(sug.confidenceScore)} ${confidenceColor(sug.confidenceScore)}`}>
                ...
                {isSelected && (
                  <div className="px-4 pb-4 border-t border-slate-100 pt-4 space-y-4">
                    ...
                    <div className="bg-amber-50 rounded-lg p-3 border border-amber-100">
                      <h5 className="text-xs font-semibold text-amber-800 mb-2">🔍 归因分析</h5>
                      <p className="text-xs text-amber-700 mb-2"><strong>主因：</strong>{sug.attributionAnalysis.primaryFactor}</p>
                      <p className="text-xs text-amber-700 mb-2"><strong>促成因素：</strong>{sug.attributionAnalysis.contributingFactors.join('、')}</p>
                      <div className="mt-2">
                        <p className="text-xs text-amber-600 mb-1">模态贡献度：</p>
                        <div className="space-y-1">
                          {Object.entries(sug.attributionAnalysis.modalityContributions).map(([modal, contrib]) => (
                            <div key={modal} className="flex items-center gap-2">
                              <span className="text-xs text-slate-600 w-24 truncate">{modal.split('_').map(w => w.charAt(0).toUpperCase() + w.slice(1)).join(' ')}</span>
                              <div className="flex-1 bg-amber-100 rounded-full h-1.5">
                                <div className="bg-amber-500 h-1.5 rounded-full" style={{ width: `${contrib * 100}%` }} />
                              </div>
                              <span className="text-xs text-slate-500 w-8">{Math.round(contrib * 100)}%</span>
                            </div>
                          ))}
                        </div>
                      </div>
          ...
          {!selected ? (
            ...
            <div className="bg-white rounded-xl border border-slate-200 shadow-sm p-5 space-y-4">
              <div className="flex items-center justify-between">
                ...
                <span className={`text-xs px-2 py-0.5 rounded-full ${priorityConfig[selected.priority].bg}`}>{priorityConfig[selected.priority].label}</span>
              </div>
              ...
              <div className="p-4 bg-emerald-50 rounded-lg border border-emerald-200">
                ...
                <p className="text-xs text-emerald-600 mb-3">采纳后将自动生成交付干预日志，并更新课程状态</p>
                <textarea
              ...
              <div className="p-4 bg-red-50 rounded-lg border border-red-200">
                ...
                <p className="text-xs text-red-600 mb-2">拒绝原因将反哺 AI 模型训练，提升后续建议质量</p>
                <select
                  value={rejectReason}
                  onChange={(e) => setRejectReason((e.target as HTMLSelectElement).value)}
                ...
                >
                  ...
                  {rejectOptions.map(opt => <option key={opt.value} value={opt.value}>{opt.label}</option>)}
                </select>
                <textarea
                  placeholder="补充说明（可选）..."
              ...
              <div className="p-4 bg-blue-50 rounded-lg border border-blue-200">
                ...
                <p className="text-xs text-blue-600 mb-2">保留 AI 建议方向，但调整具体实施方案</p>
                <textarea
                ...
                {decisionLogs.map(log => (
                  <div key={log.id} className="flex items-start gap-3 p-3 bg-slate-50 rounded-lg">
                    <div className={`w-2 h-2 rounded-full mt-1.5 ${
                      log.action === 'approve' ? 'bg-emerald-500' : log.action === 'reject' ? 'bg-red-500' : 'bg-blue-500'
                    }`} />
                    <div>
                      <p className="text-xs font-medium text-slate-700">
                        {log.action === 'approve' ? '采纳' : log.action === 'reject' ? '拒绝' : '修改'}
                      </p>
                      <p className="text-xs text-slate-500 mt-0.5">{log.timestamp}</p>
                      {log.feedbackToAI && <p className="text-xs text-slate-400 mt-1">反馈：{log.feedbackToAI}</p>}
                    </div>
                  </div>
                ))}
              </div>
            ...
            <div className="bg-white rounded-xl border border-slate-200 shadow-sm p-5">
              ...
              <div className="space-y-2">
                {processedSuggestions.map(sug => (
                  <div key={sug.id} className="flex items-center justify-between p-3 bg-slate-50 rounded-lg">
                    <div>
                      <p className="text-xs font-medium text-slate-700">{sug.title}</p>
                      <p className="text-xs text-slate-400 mt-0.5">第{sug.week}周 · {categoryLabels[sug.category]}</p>
                    </div>
